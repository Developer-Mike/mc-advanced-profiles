...
class ModSelection(ctk.CTkFrame):
    def __init__(self, root: any, app: "App", mods: List[Mod], **kwargs):
        ...
        lb_mods = ThemedLabel(fv_header, text="Mods", font_size=30, bold=True)
        lb_mods.grid(row=0, column=0, sticky="w")

        bt_add_mod = ImageButton(fv_header, PLUS_ICON_WHITE, size=(25, 25), padding=(5, 5), command=self._add_mod_dialog)
        bt_add_mod.grid(row=0, column=1, sticky="w", padx=20)
