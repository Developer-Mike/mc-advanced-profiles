...
class ModView(ctk.CTkFrame):
    def __init__(self, app: "App", root, mod: "Mod", delete_callback: callable):
        ...
        lb_mc_version = ThemedLabel(fv_name, text=f"({', '.join(mod.mod_minecraft_version)})", font_size=15, text_color="#aaaaaa")
        lb_mc_version.pack(side=tk.LEFT, anchor="w", padx=10)

        lb_mod_description = ThemedLabel(fv_content, text=mod.mod_description[:100] + "\n" + mod.mod_description[100:200], font_size=15, text_color="#aaaaaa")
